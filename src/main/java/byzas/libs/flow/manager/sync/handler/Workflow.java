...
import byzas.libs.flow.manager.sync.exception.SyncWorkflowException;
import byzas.libs.flow.manager.sync.log.LogUtil;
                        ...
                        (f, method) -> f.flatMap(result -> {
                            if (result) {
                                ...
                                return method.doActionMono(context)
                                        .onErrorResume(t -> {
                                            logUtil.info("[WORKFLOW-STEP] [{}] completed exceptionally.", method.getName());
                                            return Mono.error(new SyncWorkflowException(t));
                                        })
                                        .doOnSuccess(d -> {
                                            logUtil.info("[WORKFLOW-STEP] [{}] completed successfully.", method.getName());
                                        });
                            ...
                            (acc, step) -> {
                                if (acc) {
                                    try {
                                        ...
                                        throw new SyncWorkflowException(e);
                                    }
                                }
                                return false;
                            },
